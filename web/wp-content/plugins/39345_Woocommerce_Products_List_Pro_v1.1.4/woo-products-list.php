...
function wc_products_list_pro($allsets) {
  ...
  if (isset($wcplpro_orderby) && $wcplpro_orderby != null) {
    ...
    if ($wcplpro_orderby == 'title' || $wcplpro_orderby == 'post__in' || $wcplpro_orderby == 'date') {
  ...
  if ( $query->have_posts()) {
    ...
    if (($wcplpro_filters_position == 'before' || $wcplpro_filters_position == 'both') || ($wcplpro_pagination == 'before' || $wcplpro_pagination == 'both')) {
      ...
      $out .= wcplpro_filters_form($wcplpro_filter_cat, $wcplpro_filter_tag, get_transient('wcpl_'. $useruniq .'_'.$wcplpro_wcplid .'_cat'), get_transient('wcpl_'. $useruniq .'_'.$wcplpro_wcplid .'_tag'), $wcplpro_wcplid, 'before');
    ...
    if (($wcplpro_filters_position == 'after' || $wcplpro_filters_position == 'both') || ($wcplpro_pagination == 'after' || $wcplpro_pagination == 'both')) {
      ...
      $out .= wcplpro_filters_form($wcplpro_filter_cat, $wcplpro_filter_tag, get_transient('wcpl_'. $useruniq .'_'.$wcplpro_wcplid .'_cat'), get_transient('wcpl_'. $useruniq .'_'.$wcplpro_wcplid .'_tag'), $wcplpro_wcplid, 'after');
    ...
    foreach($wcplpro_order as $colkey => $colname) {
      if(${$colkey} == 1 && isset($col_checker[$colkey]) && $col_checker[$colkey] == true) {
        ...
        if ($colkey == 'wcplpro_custommeta' && !empty($custom_meta_header)) {
          ...
          $head_array[$colkey] = '<th class="'. $colkey .' '. (!in_array($colkey, $skip_sorting) ? 'sortable_th' : '') .' '. $wcplpro_th_class .'" '. (!in_array($colkey, $skip_sorting) ? 'data-sort="'. (in_array($colkey, $sort_as_string) ? 'string' : 'float') .'"' : '') .'>'. ((isset($wcplpro_columns_names[$colkey]) && $wcplpro_columns_names[$colkey] != '') ? $wcplpro_columns_names[$colkey] : $headenames[$colkey]) .'</th>';
...
function wcplpro_scripts() {
  ...
  wp_register_script( 'wcplpro_js', plugins_url('assets/js/wcplpro.js',__FILE__ ), array( 'jquery', 'wcplpro_select2_js' ));
  wp_enqueue_script('wcplpro_js');
   
  $vars = array( 
    'ajax_url' => admin_url( 'admin-ajax.php' ), 
    'cart_url' => apply_filters( 'woocommerce_add_to_cart_redirect', $woocommerce->cart->get_cart_url()),
...
function wcplpro_filters_form($wcplpro_filter_cat, $wcplpro_filter_tag, $cat_transient, $tag_transient, $wcplpro_wcplid, $place){
