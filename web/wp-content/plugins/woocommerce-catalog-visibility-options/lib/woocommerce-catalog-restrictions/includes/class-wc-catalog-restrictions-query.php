...
class WC_Catalog_Restrictions_Query {
	...
	public function taxonomy_filter( $args, $taxonomies ) {
		...
		if ( in_array( 'product_cat', $taxonomies ) ) {
			...
			if ( $disallowed === false ) {
				...
				SELECT term_id FROM {$wpdb->prefix}term_taxonomy WHERE taxonomy = 'product_cat' AND term_id NOT IN (SELECT {$this->table_term_meta}.{$this->term_meta_id_field} FROM {$this->table_term_meta} WHERE meta_key = '_wc_restrictions')
				UNION ALL 
				SELECT {$this->table_term_meta}.{$this->term_meta_id_field} FROM {$this->table_term_meta} WHERE ( (meta_key ='_wc_restrictions' AND meta_value = 'public') OR (meta_key = '_wc_restrictions_allowed' AND meta_value IN ('" . implode( "','", $user_roles ) . "') ) ) )";
	...
	public function posts_where( $where, &$query ) {
		global $wc_catalog_restrictions, $wpdb, $woocommerce;
	...
	public function query_allowed_product_ids_by_role( $pre_filtered_products = false ) {
        ...
        SELECT DISTINCT $wpdb->posts.ID FROM $wpdb->posts INNER JOIN (
            SELECT DISTINCT post_id FROM $wpdb->postmeta 
                    ...
                    AND post_id NOT IN(SELECT DISTINCT tr.object_id FROM {$this->table_term_meta} 
                        INNER JOIN $wpdb->term_taxonomy tt on {$this->table_term_meta}.{$this->term_meta_id_field} = tt.term_id
            ...
            SELECT  post_id FROM $wpdb->postmeta 
                ...
                UNION ALL 
            SELECT  tr.object_id FROM {$this->table_term_meta} 
                    INNER JOIN $wpdb->term_taxonomy tt on {$this->table_term_meta}.{$this->term_meta_id_field} = tt.term_id
                    INNER JOIN $wpdb->term_relationships tr on tt.term_taxonomy_id = tr.term_taxonomy_id
